...
#[tauri::command]
pub fn init_ns_panel(app_handle: AppHandle<Wry>, window: Window<Wry>, app_shortcut: &str) {
    INIT.call_once(|| {
...
fn register_app_shortcut(app_handle: &AppHandle<Wry>, app_shortcut: &str) {
    let mut shortcut_manager = app_handle.global_shortcut_manager();
    let window = app_handle.get_window(PANEL_LABEL).unwrap();

    let panel = panel!(app_handle);
    shortcut_manager
        .register(app_shortcut, move || {
            ...
            if panel.is_visible() {
                ...
                window
                    .emit("newSessions", Some(get_tmux_sessions()))
                    .unwrap();
